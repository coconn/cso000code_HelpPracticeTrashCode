# Seth-Rifkin-KohyamaHypoth.R
# 
# help Seth with the Big Grid seedling data for his collab with Joe Wright
#
...
# bring in seedling data
# find the basal area for different species (like 40 species)
# find abundance of seeds of those species (phenology data)
# find seed survival rates
# estimate seed recruitment

# Sapling recruitment per unit stem basal area increased with declining maximum height in Panama, which is consistent with the criterion for coexistence of species of differing stature derived from Kohyama’s forest architecture hypothesis.
...
# example that looks the same
speciesnames <- c("a","a","a","a","b","b","b","b")
speciesdbh <- c(2,3,4,1,1,4,70,2)
exampletable <- data.frame(sp=speciesnames,dbh=speciesdbh)
exampletable$sp <- as.character(exampletable$sp)

# CTFS functions to use
# first function
...
# make new ba column
exampletable$ba <- ba(exampletable$dbh,dbhunit='cm')

# basum for all individuals - NOT what we want
exampletable$basum <-basum(exampletable$dbh,mindbh=0.5,dbhunit='cm')

# basum for all individuals of EACH species - YES what we want!
install.packages('dplyr')
library(dplyr)

datac <- ddply(exampletable, "sp", mean=mean(exampletable$speciesdbh))


ddply(exampletable,~sp,summarise,mean=mean(speciesdbh),sd=sd(speciesdbh))
aggregate(exampletable$speciesdbh, by=list(exampletable$sp), FUN=mean)[2]


cdata <- ddply(exampletable, exampletable$sp, summarise,
               N    = length(speciesdbh),
               mean = mean(speciesdbh),
               sd   = sd(speciesdbh)
)
...
# calculate basal area for each species
